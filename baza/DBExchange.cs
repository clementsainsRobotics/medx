   ...
   public class DBExchange
    {
       ...
       public string versionNumber = Application.ProductVersion.ToString();
       ...
       public DataTable tlServiceTable;
       ...
       public bool MkbDataSet;
       public DataTable tlPatientTicket;
...
#region Singleton
       ...
       public bool checkUserPassword(string pass)
       {
           ...
           MD5 md5h = MD5.Create();
           byte[] PassHash = md5h.ComputeHash(Encoding.Default.GetBytes(pass));
           StringBuilder crHash = new StringBuilder();
           foreach (byte hashByte in PassHash)
           {
               crHash.Append(String.Format("{0:x2}", hashByte));
           ...
           byte[] UNameHash = md5h.ComputeHash(Encoding.Default.GetBytes(dBUserName));
           StringBuilder crHashN = new StringBuilder();
           foreach (byte hashByte in UNameHash)
           {
               crHashN.Append(String.Format("{0:x2}", hashByte));
           ...
           string dBLoginHash = crHashN.ToString();
           string strConnect = "Server=" + dbServerString + ";Port=5432;User Id=" + dBUserName + "; Password=" + dBPassHash 
               + ";Database=medex; searchpath=medex;";
          
           NpgsqlConnection connectDb1 = new NpgsqlConnection(strConnect);
           try
           {
               connectDb1.Open();
               //  NpgsqlCommand command = new NpgsqlCommand(strConnect, mDBConnection);
               if (connectDb1.State == ConnectionState.Open)
               {
                   gotPass = true;
                   connectDb1.Close();
               }
       ...
       public void changePass(string pass)
       {
           ...
           try
           {
               ...
               {
                   NpgsqlCommand cpass = new NpgsqlCommand("ALTER USER "+dBUserName+" WITH PASSWORD '"+dBPassHash+"' ;" ,connectDb);
                   cpass.ExecuteNonQuery();
                   connectDb.Close();
                   DbInitConnection(dbServerString, dBUserName, pass);
                   System.Windows.Forms.MessageBox.Show("Пароль успешно изменён");
               }
               else
               {
                   System.Windows.Forms.MessageBox.Show("Извините, соединение потеряно. \nПовторите операцию ещё раз после его восстановления.");
               }
       ...
       public bool CheckProgramVersion()
       {

           NpgsqlCommand getProgVer = new NpgsqlCommand("Select MAX(param) from settings where prog_oid > 1 ;" ,connectDb);
           bool needUpdate = false;
           try 
           {
               int dbProgVer = Convert.ToInt32((string)getProgVer.ExecuteScalar());
               int internalProgVer = Convert.ToInt32(versionNumber.Replace(".",""));
               

               if (dbProgVer > internalProgVer)
               {
                   needUpdate = true;           
               }
       ...
       //\ пробую что нибудь напечатать
       //    получается не то чтобы очень слишком далеко расположены кнопки по сравнению со старой клавой, 
       //        приходится немного дальше располагать пальцы, это не так сложно но тем не менее совсем не привычно, 
       //а главное я ухожу стого пути который сам для себя проложил раньше, не знаю куда мне двигаться теперь оставить эту клаву или нет
       //    руки смещены влево потому что правая часть упирвется в кресло, с этим ничего не сделать вообще
       //        поэтому перекос влево, то есть правая сторона находится не совсем там где надо
       //            на микрософтовой я думаю также будет беда со слишком далёким расположением кнопок
       // кнопки там не менее мягкие, даже ещё мягче и ход у них меньше, единственный баг что все таки не хватает многих кнопок в принципе
       //     во вторых это проблема беспрводной связи, что все таки оно будет находиться почти на причинном месте и с этим трудно смириться
       //         можно конечно ничего не менять и взять такую же старую клаву как моя, только черную, кнопки там для меня настолько привычны 
       // просто не сравнить ни с чем, на микрософте из плюсов только вероятное удобство при написание каких то длинных страниц
       // изогнутая клава это в принципе возможно реальный плюс, поехать завтра и взять вторую клаву?
       // сказать что эта для программинга, а та для чего?
       // моя диалоговая на самом деле не намного хуже, единственно она не так удобно лежит на коленках, их приходится немного поджимать
       // и это реальный баг, на к300 есть пара кнопок для слэшей - удобно писать комменты, минусы я описал, есть пачка доп кнопок для управления мультимедия 
       // по функциональным кнопкам
       // в принципе конечно достаточно удобно писать нечто, особенно что есть нбольшая подставка для рук, но приходится тянуться до некоторых кнопок
       // у меня мелкие пальцы видимо, поэтому я не совсем может быть достаю.
       // и это был один из поводов для выбор диалоговой клавиатуры, она теперь продается в черном цвете, кнопки там белые, но смысл тот же самый
       // я могу снова взять её в нагрзку, до Ё в принципе очень сложно достать, но кнопки явно мягче диалога, хотя это палка о двух концах естественно, 
       // но вероятно срок службы выше, на диалоге основная пробелма в том что кнопки начинают глючить просто от грязи, кое что получается набирать вслепую, не знаю хорошо или плохо., 
       // левая рука практически постоянно на месте, а вот правой приходится сновать из угла в угол, нажиматьзапятые или энтер., но в целом впечатление достаочно приятное
 
       // да на диалоге немного другой ход кнопок - несолкь туговат, зато на все кнопки можно с легкостью нажать, не приходится тянуться до них, 
       // клава располагается на коленках немного не так, зато все кнопки уже попадают автоматически, 
       // главное я сразу могу нажать на энтер мизинцем и все остальные кнопки у меня также в пределах досягаемости, тут правда приходится проявлять дополнительный контроль, но тут уж ничего не поделаешь
       // от тог что руки располагаются слишком близко друг к дружке , получчается небольшое напряжение, но сейчас основная проблема диалога в том, что некоторые кнопки то ли заедют, то ли глючат.
       // была мысль про то что ту что то не так расположены кнопки, но понять пока не могу сути,
       // ещё поскольку это все таки хаб, то частично присутствует и то самое излучение, естественно не в той мере, как от блютуса или что там ещё, новсе таки определенное поле создается
       // баг в том , что руки приходится держать немног не свободно. а стараться держать их вместе. зато клава легко помещается на столе и естественно под вторым моником, а другая клава занимает пол стола прямо сейчас
       // конечно печатать на неё удобнее , но на своей то я тоже отпечатал уж сколько

       // второе это конечно вес самой кклавиатуры, хотя это может и не так важнго в принципе, но все таки... не знаю надо что-то решить будет, нажимать на кнопки достаточно удобно,
       // кажется это программерская клавка или писательская, хотя ля писания лучше может быть взять мелкософтовую, потому что я думаю оно все-таки сподручнее будет
       // хотя окнечно оченно дорого, даже не знаю что делать, была к300 немного короче, потому что приходится смещать руки... 
       // с другой стороны не требуется большой концентрации для того чтобы попасть по кнопкам, оно достаточно большое
       // стрелки оказались достаточно удобными несмотря на первичные опасения, что будут проблемы чтобы их достать, нет всё в принципе нормально
       // в целом всё здорово кроче того что все таки не совсем удобно нажимать на кнопки, может просто от непривычки не знаю, вроде всё в норме, но сама клава дюже большая и упирается 
       // , упирается упирается...

       // есть тема положить между коленок и писать таким образом тут как раз решает то что есть спец подставка для рук, хотя конечно это не микрософт нэчурал, 
       // но все таки руки уже не в висячем положении и их можно отдохнуть немного, в общем определнно что-то в этом есть, но правда в том что это опять таки может быть не тот путь котрый я по-настоящему хотел, 
       // ведь хотел то я взять беспроводную, а вчера поехал к миту за клещами, чтобы поехать сделать Дине интернет, оказалось я отдал ей свой последний металлический коннектор, Мит сказал, их сложно найти
       // и мне в итоге самому не хватило, но когда я приехал к Миту он сказал, что беспроводные девайсы он старается избегать, я ведь вообще-то тоже, даже свой трэкбол я хотел переделать, 
       // чтобы он был с проводом, чтобы не было дополнительного не нужного мне облучения, но прилепить провод я просто не понял куда, хоть и разобрал мышь, кстати у меня есть сейчас цельных две беспроводных мыши из которых можно попробовать сделать одну проводную
       // мне говорят чтобы я занимался админством, даже брат привез мне видяху, привез пару компов, привезсенрвак и винты к нему, он как ждал. Н о наверное не понимает что мне нужно чего-то другого, не такой работы я ищу, что-то где мог бы делать полезное, использовать свой мозг полностью
       // не знаю что это может быть, где я мог использовать свой мозг на 100% иногда я встаю на элементарных задачах, а потом начинаю метаться из стороны в сторону, пытаясь исправить свою ошибку, всё это порядком поднадоело.
          ...
          if (chkPassport == false)
          {
              ...
              try
              {
                  ...
                  NpgsqlCommand CreateNewPatient = new NpgsqlCommand("INSERT INTO patient_list "
                                  ...
                                  + "country, town_id, area, street_n, house, building, flat, address, doc_ser, doc_num, doc_org, doc_date "+ iPhone1 + iPhone2 + iPhone3 + comment + trustMan + ")" 
       ...
       /// <summary>
       /// Изменение запписи существующего пациента
       /// нужно сделать проверку на повторы
       /// </summary>
       /// 
       public void execSqlCommand(string ExecString)
       {
           try
           {
                   NpgsqlCommand CreateNewPatient = new NpgsqlCommand(ExecString, connectDb);
                   CreateNewPatient.ExecuteNonQuery();
                   writeNewPatient.Clear();
               }
               catch (Exception exception)
               {
                   System.Windows.Forms.MessageBox.Show(exception.Message.ToString()+" execSqlCommand");
               }
               finally
               {
       ...
       ///
       /// Получает список назначений для пациента
       /// 
       public List<string> rtnPatTcktLst(Int32 thisPatId)
       {
           List<string> patTicketList = new List<string>();

           try
           {
               patTicketList.Clear();
               NpgsqlDataAdapter loadTickets = new NpgsqlDataAdapter("Select (select trim(name) from reestr where reestr.reestr = ticket.service_id),service_id,ticket_id, date_in, "
               + "date_app, date_out, (select trim(family_name) from doctors where doc_id = ticket.doc_in ) as family_name, doc_in, doc_app, doc_out from ticket where pat_id = " + thisPatId + " and date_out is null order by date_in ASC", DBExchange.Inst.connectDb);
               tlPatientTicket = new DataTable();
               loadTickets.Fill(tlPatientTicket);
               foreach (DataRow roww in tlPatientTicket.Rows)
               {
                   patTicketList.Add(((DateTime)roww[3]).ToShortDateString() + " " + (string)roww[0] + " " + (string)roww[6]);
               }
           }
           catch (Exception exception)
           {
